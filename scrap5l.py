...
def register_automaton(Location, Register, start):
    return RegisterAutomaton(start = start,
                             ...
                             update = z3.Function('update', Location, Label, Register, Register),
...
for n, a, c in trie.transitions():
   ...
   transition_constraints.append(z3.If(z3.Exists([r], z3.And(r != fresh, m.val(n.node, r) == a.value)),
                                       z3.ForAll([r], z3.Implies(z3.And(r != fresh, m.val(n.node, r) == a.value),
                                                                 z3.Xor(
                                                                    z3.And(ra.transition(m.map(n.node), a.label, r) == m.map(c.node),
                                                                        ra.guard(m.map(n.node), a.label, r) == True),
                                                                    z3.And(ra.transition(m.map(n.node), a.label, fresh) == m.map(c.node),
                                                                           ra.guard(m.map(n.node), a.label, r) == False)
                                                                        ))),
   ...
   transition_constraints.append(z3.ForAll([r, rp],
             z3.Implies(z3.And(ra.transition(m.map(n.node), a.label, r) == m.map(c.node),
                               ra.guard(m.map(n.node), a.label, r) == True
                               ),
                        z3.If(r != fresh,
                              m.val(c.node, r) == m.val(n.node, r),
                              z3.Implies(ra.update(m.map(n.node), a.label, r) == rp,
